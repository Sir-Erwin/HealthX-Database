...
CREATE DEFINER = CURRENT_USER TRIGGER `healthx`.`APPOINTMENT_BEFORE_INSERT` BEFORE INSERT ON `APPOINTMENT` FOR EACH ROW
  ...
  SELECT PATIENT.PID, PATIENT.PCP_EID
  INTO PID , PCP_EID 
  FROM PATIENT
  WHERE PATIENT.PID = NEW.PATIENT.PID;
 
  IF  PCP_EID IS NULL OR  PCP_EID!= NEW.PCP_EID THEN
	  SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT= 'Patient requires approval from primary care physician';
  END IF;
END
